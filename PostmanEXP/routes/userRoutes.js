...
router.post("/checkout", auth.verify, (req, res) => {
  userController
    .createOrder(req.headers.authorization, req.body)
    .then((resultFromController) => {
...
router.put("/userData", auth.verify, (req, res) => {
  const userData = auth.decode(req.headers.authorization); // get user from token
  if (!userData) {
    return res.status(401).send("Unauthorized");
  }

  userController
    .updateUserData(userData.id, req.body)
    .then((updatedUser) => {
